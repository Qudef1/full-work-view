...
def render_heyreach_tab():
    ...
    st.markdown(
        "Полный функционал работы с HeyReach данными реализован в отдельном сервисе:"
    )
    st.link_button(
        "🚀 Открыть HeyReach Data Filtration",
        "https://heyreach-data-filtraition.streamlit.app/",
        use_container_width=True,
    )
    st.markdown("---")
    st.markdown(
        "Или используйте форму ниже для быстрой загрузки и фильтрации CSV:"
    )
...
def render_patent_tab():
    ...
    st.markdown(
        ...
        " или пустой запрос для lens [Пустой запрос](https://www.lens.org/lens/search/patent/list?p=0&n=10&s=_score&d=%2B&f=false&e=false&l=en&authorField=author&dateFilterField=publishedDate&orderBy=%2B_score&presentation=false&preview=true&stemmed=true&useAuthorId=false)",
    )
    st.markdown(
